...
public class ApiProxyServlet extends HttpServlet {
    ...
    /**
     * HttpClient v4.1 doesn't have the {@link org.apache.http.util.EntityUtils#consumeQuietly(org.apache.http.HttpEntity)}
     * method.
     */
    ...
    /**
     * These are the "hop-by-hop" headers that should not be copied. http://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html I use
    ...
    /**
     * Take any client cookies that were originally from the proxy and prepare them to send to the proxy. This relies on cookie
     * headers being set correctly according to RFC 6265 Sec 5.4. This also blocks any local cookies from being sent to the proxy.
     */
    ...
    /**
     * Reads the request URI from {@code servletRequest} and rewrites it, considering targetUri. It's used to make the new
     * request.
     */
    ...
    /**
     * For a redirect response from the target server, this translates {@code theUrl} to redirect to and translates it to one the
     * original client can use.
     */
    ...
    private void addAuthToHeader(HttpRequest proxyRequest) {
        ...
        if (authValue != null) {
            proxyRequest.setHeader("Authorization", authValue);
        }
    }
