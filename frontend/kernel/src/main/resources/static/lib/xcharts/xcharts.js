...
function colorClass(el, i) {
  ...
  return ((c !== null) ? c.replace(/color\d+/g, '') : '') + ' color' + ai;
}
  //self fix: fix color
//function colorClass(el, i) {
//  var c = el.getAttribute('class');
 ...
 *
 */
function xChart(type, data, selector, options, setResize) {
  var self = this,
    resizeLock;
...
/**
 * define will cause an error because require has a function called define
 */
